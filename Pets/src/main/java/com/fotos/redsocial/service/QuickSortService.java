 ...
 * - ordena de menor a mayor los shelters en base a la capacidad máxima de animales que soporta.
 * 
 * CASO DE USO: "Un grupo de rescatistas acaba de salvar a cierta cantidad de perros de una situación de maltrato y
 * necesitan ser distribuidos urgentemente entre los refugios de la red de nuestra aplicación, y
 * el coordinador necesita tomar decisiones rápidas y eficientes sobre dónde alojar a estos animales.
 * Por lo tanto, ordena aquellos shelters que tienen mayor / menor capacidad para optimizar la distribución."
 *
 *
 * FUNCIONAMIENTO:
...
public class QuickSortService {
    ...
    public List<ShelterDTO> sortSheltersByCapacity() {
        // 1. Obtener todos los refugios de la base de datos.
        List<Shelter> shelters = shelterRepository.findAll();
        if (shelters == null || shelters.isEmpty()) {
            return Collections.emptyList();
        }
        // 2. Llamar al método de ordenamiento para que actúe sobre la lista.
        quickSort(shelters, 0, shelters.size() - 1);
        
        // 3. Convertir la lista ordenada de entidades a una lista de DTOs para la respuesta.
    ...
    private void quickSort(List<Shelter> shelters, int low, int high) {
        ...
        // 1. Utilizar una pila (Deque como stack) para la implementación iterativa.
        Deque<int[]> stack = new ArrayDeque<>();
        // 2. Meter el rango inicial (la lista completa) en la pila.
        ...
        while (!stack.isEmpty()) {
            ...
            // 4. Realizar la partición del sub-array. `pi` es el índice final del pivote.
            int pi = partition(shelters, l, h);

            // 5. Si hay elementos a la izquierda del pivote, meter ese rango en la pila.
            if (pi - 1 > l) {
                stack.push(new int[] { l, pi - 1 });
            }
            // 6. Si hay elementos a la derecha del pivote, meter ese rango en la pila.
            if (pi + 1 < h) {
    ...
    private int partition(List<Shelter> shelters, int low, int high) {
        ...
        // 2. `i` es el índice del último elemento que era más pequeño que el pivote.
        int i = (low - 1);
        
        // 3. Recorrer el sub-array desde `low` hasta `high-1`.
        for (int j = low; j < high; j++) {
            // 4. Si el elemento actual es menor o igual al pivote...
        ...
        // 5. Colocar el pivote en su posición final correcta, intercambiándolo con
        // el elemento en `i+1`.
        Collections.swap(shelters, i + 1, high);
        
        // 6. Devolver el índice de partición (la posición final del pivote).
        return i + 1;
    }
}